...
def record_telemetry(
    ...
    with _TELEMETRY_LOCK:
        ...
        record: dict[str, Any] = {
            ...
            "applications": [dict(a) if not isinstance(a, dict) else a for a in (payload.applications or (existing.get("applications", []) if existing else []))],
            "device_info": payload.device_info or (existing.get("device_info") if existing else None),
            "uptime_info": payload.uptime_info or (existing.get("uptime_info") if existing else None),
            "foreground_app": payload.foreground_app or (existing.get("foreground_app") if existing else None),
...
def get_telemetry_for_device(
    ...
    return EndpointTelemetryOut(
        ...
        device_info=rec.get("device_info"),
        uptime_info=rec.get("uptime_info"),
        foreground_app=rec.get("foreground_app"),
